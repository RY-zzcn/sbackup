...
选项:
  ...
  --skip-runtime-update   不检查/更新 Restic
...
if [[ $runtime_update == true ]]; then
  "$project_root/scripts/install-runtime-tools.sh" --restic-only --update
else
...
rm -f /usr/local/share/sbackup/deploy/systemd/sbackup-monitor.service
install -m 0644 "$project_root/deploy/systemd/sbackup-maintenance.service" "$project_root/deploy/systemd/sbackup-maintenance.timer" /usr/local/share/sbackup/deploy/systemd/
